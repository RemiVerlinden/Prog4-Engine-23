...
void engine::SpawnerComponent::Initialize()
{
	EventSubscribe(OnGameObjectSpawn);
}

void engine::SpawnerComponent::AddSpawnableObject(Prefab&& spawner)
{
...
void engine::SpawnerComponent::SpawnGameObject(const std::string& objectName, Scene* pScene)
{
...
GameObject* engine::SpawnerComponent::GetPrefabObject(const std::string& objectName)
{
	if (m_SpawnableObjects.find(objectName) != m_SpawnableObjects.end())
...
void engine::SpawnerComponent::OnEvent(const Event& sent)
{
